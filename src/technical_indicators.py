...
# Load your dataset
df = pd.read_csv(current_dir / "donnees.csv", parse_dates=["date"])
...
df["close"] = df.groupby("PERMNO")["y"].apply(compute_close).reset_index(level=0, drop=True)
...
df.drop(columns=["close", "EMA_Close", "MACD_diff", "MACD_Signal", 'y_forward'], inplace=True, errors='ignore')
# df.drop(columns=["EMA_Close", "MACD", "MACD_Signal", 'y_forward'], inplace=True, errors='ignore')
...
df.to_csv(current_dir / "donnees_technique.csv", index=False)

print(df)

# # Choose a specific stock PERMNO, e.g., 10
# permno_to_check = df["PERMNO"].unique()[10]  # or replace with a known ID, like 10101

# # Filter and sort
# df_check = df[df["PERMNO"] == permno_to_check].sort_values("date")

# # Print last 10 rows
# print(df_check[['date', 'close', 'y']].tail(10))
